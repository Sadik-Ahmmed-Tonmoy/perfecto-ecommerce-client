...
const MyAddresses = () => {
    ...
    const handleDeleteAddress = (id) => {
        axiosSecure.delete(`delete_address/${id}`)
        .then((res) => {
            if (res.data.status === true) {
                setIsResponseLoading(false);
                // addTokenToLocalStorage(res.data.data.token);
                        ...
                        {userAddress?.data?.map((address) => (
                            <div key={address.id} className="border border-[#E2E8F0] bg-[#fff] rounded-lg p-6 relative mb-3">
                                ...
                                <div className="flex flex-wrap items-center gap-1">
                                    <label
                                        className="flex items-center hover:cursor-pointer gap-3 text-[#252728] font-inter text-sm font-normal leading-normal w-fit h-min mt-3"
                                    ...
                                    >
                                        ...
                                        /> */}
                                            <input
                                                type="checkbox"
                                                className={`rounded h-4 w-4 ${
                                                    address?.status == 1 ? "bg-[#5DC9F4] text-white" : "bg-white"
                                                } border border-[#0094CF] appearance-none`}
                                            />
                                    ...
                                    {
                                    isResponseLoading && address?.id == selectedCheckboxes && <ProgressBar
                                    visible={true}
                                    height="30"
                                    width="50"
                                    color="#5DC9F4"
                                ...
                                <div className="absolute top-2 xs:top-3 right-4 flex items-center gap-">
                                    ...
                                    <span onClick={() => handleDeleteAddress(address?.id)} className="text-black font-inter text-sm font-medium leading-normal flex items-center gap-2 hover:cursor-pointer">
                                        <RiDeleteBin6Line />
